...
int run_cascade(void* cascade, float* o, int r, int c, int s, void* vppixels, int nrows, int ncols, int ldim)
{
	...
	//printf("offset = %d ntrees = %d tdepth = %d\r\n",offset, ntrees, tdepth);
	*o = 0.0f;
	static int printcounter=0;
	static int printcounter1 = 0;
	for(i=0; i<ntrees; ++i)
	{
		//
		tcodes = ptree - 4;
		...
		int k0,k1,x,y;
		for(j=0; j<tdepth; ++j){
			k0 = (r+tcodes[4*idx+0]*s)/256*ldim+(c+tcodes[4*idx+1]*s)/256;
			k1 = (r+tcodes[4*idx+2]*s)/256*ldim+(c+tcodes[4*idx+3]*s)/256;
			y = (r+tcodes[4*idx+0]*s)/256;
			x = (c+tcodes[4*idx+1]*s)/256;
			idx = 2*idx + (pixels[k0]<=pixels[k1]);
			if (printcounter1<100){
				printf("tcode[%i,%i,%i,%i]\r\n",tcodes[4*idx+0],tcodes[4*idx+1],tcodes[4*idx+2],tcodes[4*idx+3]);
				printf("[%d,%d,%d] x=%d y=%d k0=%i, k1=%i, pix[k0]=%d, pix[k1]=%d, idx=%i\r\n",r,c,s,x,y,k0,k1,pixels[k0],pixels[k1],idx);
			}
		}
		printcounter1++;
		//经过6级深度的决策树后idx的范围在64~127之间
		*o = *o + lut[idx-(1<<tdepth)];
		if (printcounter++<100)
		{
			printf("o = %f thr = %f idx = %d\r\n",*o, thr,idx-(1<<tdepth));
		}
