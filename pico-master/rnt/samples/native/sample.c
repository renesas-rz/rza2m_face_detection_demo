...
void write_img_to_bin_file(char* src, int size)
{
	FILE * outfile;

    outfile = fopen("imgBin", "wb" );
	
	if (outfile == NULL)
	{
		printf("open imgBin file failed\r\n");
		return;
	}

	fwrite( src,  sizeof( unsigned char ), size, outfile );
	fclose(outfile);
}
...
void process_image(IplImage* frame, int draw)
{
	...
	*/
	...
	// get grayscale image
	printf("frame->nChannels = %d\r\n",frame->nChannels);
	if(frame->nChannels == 3)
		cvCvtColor(frame, gray, CV_RGB2GRAY);
	else
		cvCopy(frame, gray, 0);

	//write_img_to_bin_file(gray->imageData, gray->width * gray->height);
	//printf("save input image as gray\r\n");
	//cvSaveImage("in_gray.jpg", gray, 0);
	...
	else
	{
		...
		printf("width=%d height=%d ldim = %d\r\n",ncols,nrows,ldim);
		//
...
#else
	//load image from array(24bitRGB)
		unsigned char* ptr = (unsigned char*)imgBin;
		img = cvCreateImageHeader(cvSize(204,274),IPL_DEPTH_8U,1);
		cvSetData(img,ptr,204);
#endif
		process_image(img, 1);
