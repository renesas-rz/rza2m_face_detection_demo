...
/*
	regression trees
*/
//在iind为索引的图像上的r,c,s区域内，取出tcode定义的2个点
//返回1：当点1的值<=点2的值
//返回0：当点1的值>点2的值
...
/*
  迭代函数生成树的节点：采样点集及树的ground truth表
 int32_t tcodes[]: <输出> 随机生成的采样点集，每个int32包含2个点的坐标(y,x)(y,x)
 float lut[]: <输出> 每个随机采样点对应的权重值
 int nodeidx: <输入> 当前节点索引
 int d:	<输入>	当前深度
 int maxd: <输入> 每个训练stage最大的深度
 float tvals[]: <输入> 标识是正阳样本还是负阳样本
 int rs[]: <输入> 经过采样测试的正阳样本，负阳样本的信息-y坐标
 int cs[]: <输入> 经过采样测试的正阳样本，负阳样本的信息-x坐标
 int ss[]: <输入> 经过采样测试的正阳样本，负阳样本的信息-直径
 int iinds[]: <输入> r,c,s标定区域对应的图片索引
 double ws[]: <输入> 每个测试样本的权重参数
 int inds[]: <输入> inds[i] = i, 范围从0~ninds
 int ninds:  <输入> 正阳样本数+负阳样本数
 int8_t* bbox：<输入> y_min, y_max, x_min, x_max (默认-127,127,-127,127)
*/
int grow_subtree(int32_t tcodes[], float lut[], int nodeidx, int d, int maxd, float tvals[], int rs[], int cs[], int ss[], int iinds[], double ws[], int inds[], int ninds, int8_t* bbox)
{
	...
	if(d == maxd)
	{
		...
		//ninds是最后一层节点经过多级二叉树分裂后的样本集，是最初传入样本集的子集
		for(i=0; i<ninds; ++i)
		{
...
/*
  参数：
 int32_t tcodes[]: <输出> 当前树的节点信息，当d=6时是63个，每个节点包含2个归一化的坐标值
 float lut[]: <输出> 当前树的ground truth表，当d=6时是64个
 int d: <输入> 当前树的深度，决定模型大小和学习及执行时间，目前设定为6
 float tvals[]: <输入> 标识是正阳样本还是负阳样本
 int rs[]: <输入> 经过采样测试的正阳样本，负阳样本的信息-y坐标
 int cs[]: <输入> 经过采样测试的正阳样本，负阳样本的信息-x坐标
 int ss[]: <输入> 经过采样测试的正阳样本，负阳样本的信息-直径
...
/*
  遍历决策树，判断是否是人脸，如果是，返回每一级树对应LUT值的累加值
 float *o: <输出> 遍历决策树后得到的阈值的累加和，它的值是每个树LUT值的累加
 int r: <输入> y坐标
 int c: <输入> x坐标
 int s: <输入> 直径 
 int <返回值> 1: 是人脸， -1：不是人脸
*/
...
float learn_new_stage(float mintpr, float maxfpr, int maxntrees, float tvals[], int rs[], int cs[], int ss[], int iinds[], float os[], int np, int nn)
{
	...
	while(ntrees<maxntrees && fpr>maxfpr)
	{
		...
		//
		thresholds[ntrees-1] = threshold;   //gang.liu

		printf("	** tree %d (%d [s]) ... stage tpr=%f, stage fpr=%f threshold=%f\n", ntrees, (int)(getticks()-t), tpr, fpr, threshold);
		fflush(stdout);
	}
...
/*
  利用已标定的人脸构建正阳数据集，利用随机区域测试出的误检人脸构建假阳数据集
 float tvals[]: <输出> 标识是正阳样本还是负阳样本，正阳为1.0f，假阳为-1.0f
 int rs[]: <输出> 经过采样测试的正阳样本，负阳样本的信息-y坐标
 int cs[]: <输出> 经过采样测试的正阳样本，负阳样本的信息-x坐标
 int ss[]: <输出> 经过采样测试的正阳样本，负阳样本的信息-直径 
 int iinds[]: <输出> r,c,s标定区域对应的图片索引
 float os[]: <输出> 每个样本遍历决策树后得到的阈值，它的值是每个树LUT值的累加
 double ws[]: 每个测试样本的权重参数
 int inds[]: inds[i] = i, 范围从0~ninds
 int ninds:  正阳样本数+负阳样本数
 int8_t* bbox： y_min, y_max, x_min, x_max (默认-127,127,-127,127)
*/
	...
	#pragma omp parallel
	{
		...
		while(!stop)
		{
			...
			*/
			...
			//判断随机的s会不会比随机图片iind还大？如果超出图片则认为取样不合理，不需要将nw++
			if ((r-(s>>1)<0) || (r+(s>>1)>=pdims[iind][0]) ||
			   (c-(s>>1)<0) || (c+(s>>1)>=pdims[iind][1]))
			   continue;
			//
...
int learn_a_cascade(char* savepath, int8_t bb[], int _tdepth, float stagetpr, float stagefpr, int maxntreesperstage)
{
	...
	for(i=0; i<cascade_nums; ++i)
	{
		//
		printf("* scanning in progress (%d/%d)\n",i,cascade_nums);
...
const void howto()
{
	printf("1. training with default params\n");
	printf("   ./picolrn <trdata> <cascade-write-path>\n");
	printf("2. training with customized params\n");
	printf("   ./picolrn <trdata> <output-cascade> <tpr> <fpr> <ntrees> <cascade_nums>\n");
	printf("3. training N new stage and append to an existing cascade\n");
	printf("   ./picolrn <current-cascade> <trdata> <tpr> <fpr> <ntrees> <cascade_nums> <new-cascade>\n");
	printf("4. initializing a new cascade with 0 trees\n");
	printf("   ./picolrn <bbox[0]> <bbox[1]> <bbox[2]> <bbox[3]> <tdepth>\n");
}
...
int main(int argc, char* argv[])
{
	...
	{
		int maxntreesforstage;
		float stagetpr, stagefpr;
		/*
			* training with customized params
			* args: <trdata> <output-cascade> <tpr> <fpr> <ntrees> <cascade_nums>
		*/
		if(!load_training_data(argv[1]))
		{
			printf("* cannot load training data ...\n");
		...
		sscanf(argv[6], "%d", &cascade_nums); 
		int8_t bb[] = {-127, +127, -127, +127};
		learn_a_cascade(argv[2], bb, 6, stagetpr, stagefpr, maxntreesforstage);
	...
	{
		/*
			* append N new stage to an existing cascade
			* args: <current-cascade> <trdata> <tpr> <fpr> <ntrees> <cascade_nums> <new-cascade>
		*/
		float stagetpr, stagefpr;
