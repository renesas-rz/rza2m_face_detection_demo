...
int32_t R_BCD_CameraCaptureStart(void)
{
    ...
    {
        /* set buffer */
        R_RVAPI_SetBufferMIPI((uint8_t)count, &frame_buffer[buff_write_point][0]);
...
static void MipiInt_isr(e_mipi_interrupt_type_t int_type)
{
    ...
    if ((int_type & VIN_INT_FIELD) == VIN_INT_FIELD)
    {
        ...
        vdc_captured = (int32_t)mb_manage_table[info.vin_nowcapturebase];
        ...
        if (isr_toggle == 0)
        {
        	PerformSetStartTime(IPP_CAMERA_FRAMERATE);
        	isr_toggle++;
        }
        else
        {
        	PerformSetEndTime(IPP_CAMERA_FRAMERATE);
        	isr_toggle = 0;
        	g_camera_vsync_us = PerformGetElapsedTime_us(IPP_CAMERA_FRAMERATE);
        }
