...
#if VIDEO_BUFFER_NUM == 1
    paddr = &video_buffer[0][0];
#else
...
#if GRAPHICS2_BUFFER_NUM == 1
    paddr = &graphics2_buffer[0][0];
#else
...
int32_t R_BCD_LcdInit(void)
{
...

    {
        ...
        info.buff_addr2 = (GRAPHICS2_BUFFER_NUM == 1)? graphics2_buffer[0] : graphics2_buffer[1];
...
void R_BCD_LcdVSync(void) 
{
    ...
    if (Req_Layer2_Swap != 0 && GRAPHICS2_BUFFER_NUM != 1)
    {
