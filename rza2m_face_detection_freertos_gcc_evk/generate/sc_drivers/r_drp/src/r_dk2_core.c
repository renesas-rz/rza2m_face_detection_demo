...
static void dk2_pre_load(unsigned char id, int zero_clear, int context_max, int state_max, unsigned char *pdesc,
    uint32_t config, uint32_t size);
static void dk2_load(unsigned char id, uint32_t tilepat, uint32_t tilesrc, uint32_t desc);
static void dk2_post_load(unsigned char id, int state_max);
static void dk2_activate(unsigned char id, uint32_t tilepat, unsigned char div);
static void dk2_pre_start(unsigned char id, unsigned char *pdesc, uint32_t param, uint32_t size);
static void dk2_start(unsigned char id, uint32_t desc);
static void dk2_unload(unsigned char id);
static void dk2_get_int(uint32_t *pdscc, uint32_t *pidif, uint32_t *podif);
...
static void reg_write(unsigned char tile, uint32_t addr, uint32_t data);
static void get_tile_info(unsigned char id, int32_t *pshift, int *pnum, uint32_t *pdma);
...
#ifdef DRP_USE_STBY
    DRP_REGSTANBY         = 0x0000003F; // 0xEAFD4018
...
#ifdef DRP_USE_STBY
    DRP_REGSTANBY         = 0x00000000; // 0xEAFD4018
    ...
    uint32_t config, uint32_t size)
{
    ...
    if (0 != zero_clear)
    {
        ...
        for (i = 0; i < 32; i++)
        {
            ...
            {
                WR64(0xEA000000 | offset | ((uint32_t)i << 11) | ((uint32_t)j << 5), 0);
            }
        }
        ...
        for (j = 0; j < 2; j++)
        {
            WR64(0xEA010000 | offset | ((uint32_t)j << 3), 0);
        }
        
        /* PeRfu */
        for (j = 0; j < 7; j++)
        {
            WR64(0xEA020000 | offset | ((uint32_t)j << 7), 0);
        }
        ...
        for (i = 0; i < 24; i++)
        {
            ...
            {
                WR64(0xEA090000 | offset | ((uint32_t)i << 11) | ((uint32_t)j << 3), 0);
            }
        }
...
#ifdef DRP_USE_STBY
    DRP_REGSTANBY = ((DRP_REGSTANBY & ~0xC0uL) | id); // 0xEAFD4018
    ...
    DRP_SFTRST &= (~(((uint32_t)id << 16) | (uint32_t)id)); /* DMACSFTRST EAFFD800 */
    DRP_CLKE   |= (((uint32_t)id << 16) | (uint32_t)id); /* DMACCLKE EAFFD810 */
...
static void dk2_activate(unsigned char id, uint32_t tilepat, unsigned char div)
{
    volatile uint32_t t0_dma;
    volatile uint32_t t1_dma;
    volatile uint32_t t2_dma;
    volatile uint32_t t3_dma;
    volatile uint32_t t4_dma;
    volatile uint32_t t5_dma;
    int32_t shift;
    uint32_t dma;
    uint32_t reg_run_sel;
    
    get_tile_info(id, &shift, 0, &dma);
    
    reg_run_sel = (0x8 | (uint32_t)shift);
...
/* Tile 5 set */
    ...
    DRP_ODIF_INTMSKCLR = (dma << shift); // 0xEAFFA024 Clear ODIF INT mask
...
static void dk2_unload(unsigned char id)
{
    ...
    DRP_ODIF_INTMSKSET = (dma << shift); // 0xEAFFA020 Set ODIF INT mask
...
static void read_evsel(int core, int state, int evidx, unsigned char *pretval, unsigned char *preaddata)
{
    ...
    if (0 == evidx)
    {
        raddr = DRPK_BASE_ADDR + (((uint32_t)core << 21) | (0x9uL << 16) | (0xBuL << 11) |
            ((uint32_t)state << 3));
    ...
    {
        raddr = DRPK_BASE_ADDR + (((uint32_t)core << 21) | (0x9uL << 16) | (0xCuL << 11) |
            ((uint32_t)state << 3));
        readdata = *(unsigned char *)raddr;
    ...
    {
        raddr = DRPK_BASE_ADDR + (((uint32_t)core << 21) | (0x9uL << 16) | (0xDuL << 11) |
            ((uint32_t)state << 3));
        readdata = *(unsigned char *)raddr;
        *pretval = (readdata & 0xF); /* readdata[3:0] */
    }
...
static void reg_write(unsigned char tile, uint32_t addr, uint32_t data)
{
    ...
    for (tile_idx = 0; tile_idx < 6; tile_idx++)
    {
        ...
        {
            *(uint32_t *)(drpk_tile_baseaddr + (addr & 0x003FFFFF)) = data;
        }
...
static void set_dma(unsigned int dma, int shift)
{
    volatile uint32_t idif_dmactli = 0xEAFF9100;
    volatile uint32_t odif_dmactlo = 0xEAFFA100;
    idif_dmactli += (uint32_t)(shift * 0x100);
    odif_dmactlo += (uint32_t)(shift * 0x100);
